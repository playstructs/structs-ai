...
- **Section indexes** markdown-link every leaf they list ([api/queries/](api/queries/), workflows, patterns, transactions, minimal schemas, troubleshooting, visuals). [IDENTITY.md](IDENTITY.md) and [USER.md](USER.md) are linked from [AGENTS.md](AGENTS.md) and [START.md](START.md). [CHANGELOG.md](CHANGELOG.md) is linked from [AGENTS.md](AGENTS.md) and the homepage footer.
- **Thin LCD/schema cards** from 1.29.0 stay as agent files but are `sitemap: false` / `noindex` so they do not grow crawled-not-indexed. `/api/queries/player` and the other pre-existing query pages stay in the sitemap.
- **Skill hrefs** publish as `/skills/<name>/SKILL.html` (no `.cursor` hop). `structs-reconnaissance` redirects to intel. [llms.txt](llms.txt) skill URLs use `.html`.
- **agentic-gaming** nav/homepage anchors are descriptive. Chain-events proto URL uses structsd `master`.
- **CI** `scripts/ci/check-inbound-links.py` requires every `sitemap.xml` URL to have an inbound `<a>` from another sitemap HTML page (`SITEMAP.md` does not count).
...
- **Webapp charting hub** — [`api/webapp/analytics.md`](api/webapp/analytics.md). Leaderboards, inventory, `GET /api/guild-bank` (+ 30-day history), `/api/denom` / `/api/resolve` / `/api/objects`, provider and agreement market snapshots, stat aggregates with LOCF, planet-activity and ledger stats, power-at-risk, planet-raid lists, census counts, `/api/block`. Amounts are strings; `_p` is base precision; optional `meta.height`.
- **`/agentic-gaming`** — answer-first page for “what is agentic gaming / what can my agent play”, with FAQPage JSON-LD, linked from the homepage nav.

### Changed

- **Guild bank is readable over HTTP.** [`guild-banking.md`](knowledge/economy/guild-banking.md) and the token-lifecycle example no longer say there is no webapp bank endpoint.
- **Envelope** — optional `meta.height` and `total`; catalog `?limit=` clamp 1–1000; stat `?bucket=` raises the max window to 30 days. [`webapp-api-protocol.md`](protocols/webapp-api-protocol.md).
- **Unpublished repo files** linked from published pages now point at GitHub blob (or `/awareness/continuity` for `memory/`, `/skills/` for the skills directory). `config/operator.md` is named as a local path; only `operator.example.md` is linked.
- **Homepage** — `WebSite.alternateName` `["Structs AI", "structs.ai"]`; Mermaid `edgeLabelBackground: #1c2128`; footer links the six previously unlinked section indexes.

### Fixed

- Redirects from retired skill URLs (`structs-economy`, `structs-mining`, `structs-power`, `structs-exploration`, `structs-diplomacy`) to the skills that absorbed them, including the `/.cursor/skills/…` forms Google still has.
- `SITEMAP.html` is `noindex` and out of `sitemap.xml` (repo manifest, not a ranking page).
...
- **Guild charter founding** — chain-global PoW (`guild-create-compute`, query `guild-charter`) alongside the one-time reactor entitlement path (`guild-create`). Documented in [hashing.md](knowledge/mechanics/hashing.md) and the [guild skill](/skills/structs-guild/SKILL.html).
...
- **Invocation lint** — new [`check-invocations.py`](https://github.com/playstructs/structs-ai/blob/main/scripts/ci/check-invocations.py), wired into CI, hard-gates three parse-time failures that the existing command-name lint cannot see: positional **arity**, `--` **order**, and the `--gas auto` rule. It reads a new `generated/structsd-signatures.txt` (emitted by an extended [`snapshot-commands.sh`](https://github.com/playstructs/structs-ai/blob/main/scripts/ci/snapshot-commands.sh)) so CI needs no `structsd` binary.
...
- **Phantom CLI commands** — `player-update-pfp-client-render-attributes` is really `player-update-pfp-cr-attributes` (corrected in [`structs-onboarding`](/skills/structs-onboarding/SKILL.html), [`action-index.md`](reference/action-index.md), [`action-quick-reference.md`](reference/action-quick-reference.md), [`actions.md`](schemas/actions.md)). `guild-membership-all-by-guild` does not exist — guild membership lives on `player.guild_id` / `player.guild_rank`; [`structs-intel`](/skills/structs-intel/SKILL.html) now uses `guild-membership-application-all` plus a Guild Stack query. `player-charge` does not exist either: **charge is derived**, `current_block_height − player.gridAttributes.lastAction`. [`scripts/BASELINE.md`](https://github.com/playstructs/structs-ai/blob/main/scripts/BASELINE.md) now carries a phantom→reality table instead of language legitimizing the lint warnings.
...
- **`llms-core.txt` budget 100 KB → 120 KB (once)** — with `defense.md` swapped in for `combat.md`, the bundle is 121,746 B. Rationale recorded in [`check-bundles.sh`](https://github.com/playstructs/structs-ai/blob/main/scripts/ci/check-bundles.sh).
...
- **Phantom `struct-all-by-planet` CLI command** — it is a Guild Stack / webapp query, not a `structsd` CLI subcommand (per [`scripts/BASELINE.md`](https://github.com/playstructs/structs-ai/blob/main/scripts/BASELINE.md)), so [`scout.sh`](https://github.com/playstructs/structs-ai/blob/main/scripts/scout.sh)'s defender query silently failed. Removed the call (defender enumeration is now pointed at the Guild Stack), and corrected the CLI-form references in the [`structs-intel`](/skills/structs-intel/SKILL.html) and [`structs-combat`](/skills/structs-combat/SKILL.html) skills and [Transcript 02](examples/transcripts/02-raid-go-no-go.md) to use `struct [commandStruct-id]` (for the Command Ship's ambit) plus a Guild Stack SQL query (for planet structs).
...
- **Mine/refine cycle lifecycle** — new "Mine/Refine Cycle Lifecycle" section in [`hashing.md`](knowledge/mechanics/hashing.md): anchors set on activation (activation *is* the cycle start; no separate "begin mining" message), cleared on deactivate/offline, **never expire** (aged = cheaper, not wedged), completion auto-restarts the cycle, and ore is checked only at completion (a refinery can run with 0 ore and count ore that arrives mid-cycle). Reflected in the [`structs-production`](/skills/structs-production/SKILL.html) skill (parallel-pipeline tip + "don't clean up aged anchors" warning), the activate/deactivate/complete effects in [`schemas/actions.md`](schemas/actions.md), and the clock entry in [`glossary.md`](reference/glossary.md).
...
- **Onboarding vs activation-code contrast** — [`auth.md`](api/webapp/auth.md) and the [`structs-onboarding`](/skills/structs-onboarding/SKILL.html) skill now state that fresh signup needs no activation code and no funds (guild fronts the fee), while activation codes only add an address/device to an existing player.
- **One-mnemonic fleet pattern** — HD path `m/44'/118'/0'/0/N` (one signup per index → fully independent players, poll `GET /structs/address/{address}` for `playerId`, no chain-imposed link/cap) added to [`team-operations.md`](playbooks/meta/team-operations.md), cross-linked from the onboarding and [`structs-permissions`](/skills/structs-permissions/SKILL.html) skills.
...
- **Staking framing** — reframed reactor staking as buying *capacity* (not a yield/APR/income stream; income is indirect via selling energy) in the [`structs-commerce`](/skills/structs-commerce/SKILL.html) skill, [`energy-market.md`](knowledge/economy/energy-market.md), [`valuation.md`](knowledge/economy/valuation.md), and the [`speculator`](identity/souls/speculator.md) soul.
...
- **Combat myth-buster visibility** — promoted "a raid cannot eliminate a player — the only prize is stored ore" to a headline callout in [`combat.md`](knowledge/mechanics/combat.md). Added `knowledge/mechanics/combat.md` and `knowledge/mechanics/energy.md` to `CORE_FILES` in [`generate-llms-full.sh`](https://github.com/playstructs/structs-ai/blob/main/scripts/generate-llms-full.sh) so the core bundle carries the combat ruleset and energy-units anchor.
...
- **`struct-trash` action** — destroys a *built* struct; irreversible; consumes build charge (`8`) via its handler (distinct from `struct-build-cancel`, which only cancels an unfinished build). Added to [`action-index.md`](reference/action-index.md), [`action-quick-reference.md`](reference/action-quick-reference.md), [`schemas/actions.md`](schemas/actions.md), [`schemas/requests.md`](schemas/requests.md), the [`permissions.md`](knowledge/mechanics/permissions.md) handler table + `PermPlay` list, [`transactions.md`](knowledge/mechanics/transactions.md), [`building.md`](knowledge/mechanics/building.md), [`conventions.md`](/skills/conventions.html), the [`structs-building`](/skills/structs-building/SKILL.html) skill, and classified as a Tier-2 irreversible action in [`SAFETY.md`](SAFETY.md).
...
- **Wildcard-token correction.** Because NATS `*` matches exactly one token and `>` matches the rest, the documented `structs.planet.*` / `structs.grid.*` single-token wildcards no longer match. Updated [`structs-streaming`](/skills/structs-streaming/SKILL.html), [`event-types.md`](api/streaming/event-types.md), [`subscription-patterns.md`](api/streaming/subscription-patterns.md), [`event-schemas.md`](api/streaming/event-schemas.md), [`api-quick-reference.md`](reference/api-quick-reference.md), and [`glossary.md`](reference/glossary.md) to use `structs.planet.{id}.*` (one planet) / `structs.planet.>` (all) and `structs.player.>` (the player subject is likewise three tokens). Added a NATS wildcard-rule callout to each.
...
- **Infusion splits 96/4** — `Infusion.GetPowerDistribution()` + `ReactorFuelToEnergyConversion = 1` (`x/structs/types/infusion.go`, `types/keys.go`): 1 ualpha → 1 mW, split so the infuser keeps `1 − commission` (default 4%, `reactor_hooks.go`) on their **own** capacity and the reactor keeps the commission. **Infusing a reactor does not power a substation** — an allocation must route capacity in. Documented in [`energy.md`](knowledge/mechanics/energy.md#creating-capacity-infusion-splits-964) and the [`structs-energy`](/skills/structs-energy/SKILL.html) skill.
...
- **Fresh-vs-aged PoW worked example** — difficulty runs 64 (fresh, impossible) → 1 once a clock reaches `range` blocks old, so an old anchor is a cheaper proof than a fresh one. Added to [`hashing.md`](knowledge/mechanics/hashing.md#worked-example-fresh-vs-aged-anchor); cross-linked from [`structs-production`](/skills/structs-production/SKILL.html) and [`structs-building`](/skills/structs-building/SKILL.html), with the mine/refine clock reset reaffirmed.
- **Defender assignment vs block vs counter** — any built, online, co-located struct can be assigned to defend regardless of ambit; same-ambit is required only to **block**, while a cross-ambit defender still **counters** (`struct_cache.go` `ResolveDefenders`). Added [Assigning Defenders](knowledge/mechanics/combat.md#assigning-defenders-struct-defense-set) to [`combat.md`](knowledge/mechanics/combat.md) and clarified the [`structs-combat`](/skills/structs-combat/SKILL.html) defense procedure.
...
- **Parallel-charge focus fire** — because charge is per-player, the only way to land many hits in one window is many accounts. Added to [`structs-combat`](/skills/structs-combat/SKILL.html) and the new team playbook.
...
- **Drift guard for the glossary** — [`scripts/check-drift.sh`](https://github.com/playstructs/structs-ai/blob/main/scripts/check-drift.sh) now validates (filesystem-only) that every glossary link target file exists and every `#anchor` resolves to a heading, so the glossary cannot silently rot.
...
- **Bundle coverage** — added `event-types.md`, `event-schemas.md`, `common-issues.md`, `glossary.md`, the new transcript, and the team playbook to [`scripts/generate-llms-full.sh`](https://github.com/playstructs/structs-ai/blob/main/scripts/generate-llms-full.sh); linked the streaming catalogs and troubleshooting from [`llms.txt`](llms.txt). Regenerated `llms-full.txt` / `llms-core.txt`.
...
- **[`conventions.md`](/skills/conventions.html)** — single source for shared skill boilerplate: transaction flags (`TX_FLAGS` / `TX_FLAGS_APPROVED`), the `--` entity-ID rule, the per-player charge bar, proof-of-work `-D` policy, one-transaction-at-a-time, and the skill-authoring template (`level` / `domain` front matter).
- **New consolidated skills** — [`structs-production`](/skills/structs-production/SKILL.html) (absorbs mining), [`structs-planets-fleet`](/skills/structs-planets-fleet/SKILL.html) (absorbs exploration), [`structs-energy`](/skills/structs-energy/SKILL.html) (absorbs power; capacity-side), [`structs-commerce`](/skills/structs-commerce/SKILL.html) (absorbs economy; market-side), [`structs-permissions`](/skills/structs-permissions/SKILL.html) (replaces diplomacy; adds delegate-agent recipes), [`structs-intel`](/skills/structs-intel/SKILL.html) (replaces reconnaissance).
- **Script toolkit** in [`scripts/`](https://github.com/playstructs/structs-ai/blob/main/scripts/README.md) — read-only helpers that turn multi-step queries into one-line decisions: [`assess.sh`](https://github.com/playstructs/structs-ai/blob/main/scripts/assess.sh), [`power-budget.sh`](https://github.com/playstructs/structs-ai/blob/main/scripts/power-budget.sh), [`scout.sh`](https://github.com/playstructs/structs-ai/blob/main/scripts/scout.sh) (raid go/no-go with the shield-vulnerability gate), [`job-status.sh`](https://github.com/playstructs/structs-ai/blob/main/scripts/job-status.sh), [`watch-defense.mjs`](https://github.com/playstructs/structs-ai/blob/main/scripts/watch-defense.mjs) (GRASS defense alerts), [`check-drift.sh`](https://github.com/playstructs/structs-ai/blob/main/scripts/check-drift.sh) (flag doc constants that drift from the live chain), and a shared [`lib.sh`](https://github.com/playstructs/structs-ai/blob/main/scripts/lib.sh).
- **Machine-readable memory** — [`memory/README.md`](/awareness/continuity.html) defines JSON schemas for operational state (`player.json`, `game-state.json`, `jobs/<job>.json`); new [`memory/jobs/`](/awareness/continuity.html) directory convention (`.json` / `.log` / `.pid` per expedition).
...
- **`llms-core.txt`** — a tight starter bundle (identity + conventions + core-loop skills + key knowledge) alongside the full `llms-full.txt`; both emitted by [`scripts/generate-llms-full.sh`](https://github.com/playstructs/structs-ai/blob/main/scripts/generate-llms-full.sh).
- **`--pfp-client-render-attributes`** support in [`create-player.mjs`](https://github.com/playstructs/structs-ai/blob/main/.cursor/skills/structs-onboarding/scripts/create-player.mjs), with local v0.18.0 validation (≤512-byte JSON object).

### Changed

- **All skills are now decision-first** with `level` / `domain` front matter and a "When to use it" + "Decisions" section. Rewrote [`structs-building`](/skills/structs-building/SKILL.html), [`structs-combat`](/skills/structs-combat/SKILL.html) (v0.18.0 raid doctrine), and [`structs-guild`](/skills/structs-guild/SKILL.html) (guild-choice criteria); refocused [`play-structs`](/skills/play-structs/SKILL.html) and [`structs-onboarding`](/skills/structs-onboarding/SKILL.html) on the new skill map and per-player charge.
- **Skill index & cross-links** updated across [`.cursor/skills/index.md`](/skills/), [`AGENTS.md`](AGENTS.md), [`SAFETY.md`](SAFETY.md), [`SITEMAP.md`](SITEMAP.md), [`llms.txt`](llms.txt), and `sitemap.xml`.
...
- **Network identifiers** standardized to `structstestnet-111` / `NETWORK_VERSION=113b` in [`TOOLS.md`](TOOLS.md), guild-stack skill, and [`structsd-install`](/skills/structsd-install/SKILL.html) (v0.17.0).
...
- **Guild stack compose** — `compose.yaml` canonical filename; default profile is `structsd structs-pg structs-sync-state structs-grass`; MCP/proxy removed from compose (agents use [Structs Desktop](https://github.com/playstructs/structs-desktop) MCP).
...
- [`structs-mining`](/skills/structs-production/SKILL.html), [`structs-building`](/skills/structs-building/SKILL.html), [`structs-combat`](/skills/structs-combat/SKILL.html) — Safety callouts now point at the reconnect flow at the point of need, so an agent reading the skill in the middle of a problem finds the verification procedure without bouncing back to async-operations.

### Context

Skilled-player feedback on the 1.10.0 ship: two real gaps. (1) The 34-hour refinery window leaves agents stranded when they reconnect — `memory/jobs/` was the persistence pattern, but no procedure told them how to *verify* what happened in the gap. (2) [`play-structs`](/skills/play-structs/SKILL.html) was supposed to be the entry-point skill but only mentioned safety in "See Also" at the bottom — new agents could run 180 lines of commands before encountering the tier framework. Both addressed.
...
- **Read-only profile** as the **default** in [`structs-guild-stack`](/skills/structs-guild-stack/SKILL.html) setup — Step 4 starts only `structsd structs-pg structs-grass` instead of the full stack. The MCP server, webapp, NATS, and signing agent must be enabled explicitly. A new "Enabling Additional Services" section documents how.
...
- **`planet-explore` split** in [`structs-exploration`](/skills/structs-planets-fleet/SKILL.html) into two distinct procedural steps: Tier 0 first-explore (routine) and Tier 2 subsequent-explore (preceded by an Approval Block surfacing `currentOre == 0`, fleet status, and the destruction warning).
- **[`structs-guild-stack`](/skills/structs-guild-stack/SKILL.html) "Lifecycle & Trust"** condensed — pin-a-tag and disable-services moved into the main Setup Procedure where they belong; the section now hosts the longer-form rationale, MCP localhost binding, signing-agent caveat, and teardown commands.
...
- **Per-skill Safety callouts** on all 11 affected skills ([`structs-onboarding`](/skills/structs-onboarding/SKILL.html), [`structs-mining`](/skills/structs-production/SKILL.html), [`structs-building`](/skills/structs-building/SKILL.html), [`structs-combat`](/skills/structs-combat/SKILL.html), [`structs-economy`](/skills/structs-commerce/SKILL.html), [`structs-energy`](/skills/structs-energy/SKILL.html), [`structs-power`](/skills/structs-energy/SKILL.html), [`structs-guild`](/skills/structs-guild/SKILL.html), [`structs-diplomacy`](/skills/structs-guild/SKILL.html), [`structs-exploration`](/skills/structs-planets-fleet/SKILL.html), [`structs-guild-stack`](/skills/structs-guild-stack/SKILL.html)). Each callout lists the skill's highest-impact ops with their tier and in-character flavor.
- **[`.cursor/skills/structs-guild-stack/SKILL.md`](/skills/structs-guild-stack/SKILL.html)** "Lifecycle & Trust" subsection — pin a release tag rather than tracking `main`, disable services not needed (read-only PG profile), bind MCP to `127.0.0.1`, signing-agent caveat, teardown commands.

### Changed

- [`AGENTS.md`](AGENTS.md), [`README.md`](https://github.com/playstructs/structs-ai/blob/main/README.md), [`index.md`](index.md), [`QUICKSTART.md`](QUICKSTART.md), [`llms.txt`](llms.txt), [`SITEMAP.md`](SITEMAP.md), and [`awareness/index.md`](awareness/index.md) — surface SAFETY.md and agent-security.md as discoverable entry points; added Critical Rule 9 in AGENTS.md.
- [`scripts/generate-llms-full.sh`](https://github.com/playstructs/structs-ai/blob/main/scripts/generate-llms-full.sh) — includes SAFETY.md and `awareness/agent-security.md` in the aggregated build.
