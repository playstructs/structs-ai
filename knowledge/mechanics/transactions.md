...
- **`StructsDecorator`** -- looks up the player for the signing address, applies the static permission check from `PermissionMap` for messages that use it, enforces the per-player message cap, and short-circuits same-block `lastAction` collisions for `ChargeMessages`. **The signing address must already be registered as a player** — if it resolves to no player, the message is rejected at ante with `"address is not registered as a player"` (ante error code 2010, `app/ante/errors.go`). This gate covers **every** `/structs.structs.*` game message; a funded-but-unregistered key can pay fees yet still be rejected. Plain Cosmos bank sends (`cosmos.bank.v1beta1.MsgSend`) are **not** Structs messages and are exempt, which is how a fresh key can be funded before it becomes a player. Client UIs should pre-check registration (`GET /structs/address/{address}` → non-`1-0` `playerId`) before offering game actions.
