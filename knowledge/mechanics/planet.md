...
- Old planet is completed and its planet structs are **destroyed** (explore requires ore already at 0 / status `complete`)
...
- **What gets stolen**: The player's `storedOre` (`StoredOreAttributeId` on the **player** object). Raids seize ore from the player who owns the planet, not from the planet itself.
- **What is NOT at risk**: The planet's unmined `buriedOre` (`BuriedOreAttributeId`, still in the ground). LCD payloads may label both as `gridAttributes.ore` — they are different attributes on different objects. Unmined ore can only be extracted by an Ore Extractor — raiders cannot touch it.
- Once ore is refined into Alpha Matter, it is cryptographically secure and cannot be stolen.
- **Fleet/CMD gate the raid, not the shield number.** Raidability is `IsDefenderCommandStructVulnerable()` (no fleet / fleet off-station / no CMD ship / CMD destroyed / CMD offline). The displayed shield value only sets raid PoW difficulty. A planet can only be raided to completion while that predicate is true (`shieldsVulnerable` is the status name for an open gate). The Command Ship defends the home planet only while the fleet is on station; an off-station fleet (for example, one you sent away to raid) opens **your** gate. Keeping the fleet on station with the Command Ship online — and well defended — is the primary raid defense. See the raid phases in [combat.md](combat.md#raid-phases-and-shields_vulnerable).
- **A raid only steals ore**: A successful raid seizes **all** `storedOre` and nothing more — it does not destroy the player or their structs. Settlement is atomic at `MsgPlanetRaidComplete`; there is no partial-loot or refund. Killing the defender's Command Ship opens the `shieldsVulnerable` window that lets the raid complete; if the defender restores or rebuilds the Command Ship before completion, the raid is rejected (`shields_active`). The `trigger_raid_defeat_by_destruction` property defeats an attacking fleet: if the **raider's** Command Ship is destroyed while away from home, the raiding fleet is defeated (`attackerDefeated`). See [combat.md — what a raid does](combat.md#what-a-raid-does).
- **Planetary defense beyond shields**: The Jamming Satellite struct provides a low-orbit ballistic interceptor network that can **evade incoming _guided_ ordnance aimed at planetary structs on their own planet, regardless of ambit** (`evadedByPlanetaryDefenses`); unguided ordnance passes through untouched. See [combat.md — Other Planetary Defense Structs](combat.md#other-planetary-defense-structs).
