...
## Charge Status
[Per-player charge bar: `charge = blockHeight − lastAction`; a **missing** `lastAction` means 0 → full charge. Note charge needed for the next planned action and ready block.]
```
...
2. Fill template with current state (use `structs_dashboard` for your snapshot, `structs_intel` for specific entities, etc.)
...
2. Read [Continuity](continuity.md) startup files: `config/operator.md`, `SAFETY.md`, `TOOLS.md`
